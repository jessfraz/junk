...
// DeleteRecordSet deletes an Azure DNS Record Set in the provided
// resource group with the given zone name.
// From: https://docs.microsoft.com/en-us/rest/api/dns/recordsets/delete
func (c *Client) DeleteRecordSet(resourceGroup, zoneName string, recordType RecordType, relativeRecordSetName string) error {
	...
	req, err := http.NewRequest("DELETE", uri, nil)
	if err != nil {
		return fmt.Errorf("Creating delete record set uri request failed: %v", err)
	}
	...
	resp, err := c.hc.Do(req)
	if err != nil {
		return fmt.Errorf("Sending delete record set request failed: %v", err)
	}
	defer resp.Body.Close()
	...
	// 204 No Content means the specified record set was not found.
	if resp.StatusCode == http.StatusNoContent {
		return fmt.Errorf("Record set with name %q was not found", relativeRecordSetName)
	}
